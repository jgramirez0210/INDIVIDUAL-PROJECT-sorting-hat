...
const displayCard = (array) => {
  ...
  array.forEach((student) => {
    ...
    <button class="btn btn-danger" data-id="${student.id}">Expel</button>
  </div>`;
  })
...
const expelStudent = () => {
  if (event.target.classList.contains('expel-button')) {
    const studentId = parseInt(event.target.getAttribute('data-'))
  }
  students.forEach(student => {
    document.getElementById('expel--${student.id}').addEventListener('click', ()=> {
      expel(student.id)
    });
  });
}
